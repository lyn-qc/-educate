...
import { count, timeStamp } from "node:console";
            ...
            page: (
                await Promise.all(
                    result.page.map(async (message) => {
                        ...
                        const reactionsWithoutMemberIdProerty = dedupedReactions.map(
                            ({ memberId, ...rest }) => rest
                        )
                        return {
                            ...message,
                            ...
                            user,
                            image,
                            reactions: reactionsWithoutMemberIdProerty,
...
export const create = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        let _conbersationationId = args.conversationId;
        ...
        if (!args.conversationId && !args.channelId && args.parentMessageId) {
            ...
            _conbersationationId = parentMessage.conversationId;
        }
        const messageId = await ctx.db.insert("messages", {
            ...
            conversationId: _conbersationationId,
